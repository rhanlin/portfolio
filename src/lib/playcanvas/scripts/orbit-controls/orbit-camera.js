...
export class OrbitCamera extends Script {
  ...
  focus(focusEntity) {
    ...
    this.distance =
      (radius * 1.5) / Math.sin(0.5 * this.entity.camera.fov * math.DEG_TO_RAD);
  ...
  _calcYaw(quat) {
    ...
    return (
      Math.atan2(-transformedForward.x, -transformedForward.z) * math.RAD_TO_DEG
    );
  }
  ...
  _calcPitch(quat, yaw) {
    ...
    return (
      Math.atan2(transformedForward.y, -transformedForward.z) * math.RAD_TO_DEG
    );
  }
}
